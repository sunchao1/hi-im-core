...
> **关联**：[理解核心1.md](./理解核心1.md)、[theme/06-群聊端到端背诵.md](../theme/06-群聊端到端背诵.md)
...
**重要补全**：msgsvr fan-out 时会对 **所有群成员所在 gateway** 各发一帧，包括 **gateway-1（甲自己）** 和 **gateway-2（乙）**。之前若只画 gateway-2 下行，链路是不全的。
...
sequenceDiagram
  ...
  Note over A,G1: ① 浏览器 → gateway（无 Hub SUB）
  A->>G1: WS 群聊 "hi,friend!"

  Note over G1,HF: ② 上行 hubclient（gateway 不 SUB）
  G1->>HF: AsyncSend wire cmd=0x030B, wire.nid=20001

  Note over HF,HB: ③ FORWARD bridge 上行
  HF->>HF: Reactor → Worker → bridge
  HF->>HB: peer.Publish(0x030B, payload)

  Note over HB,MS: ④ BACKEND SUB 表命中 msgsvr
  HB->>MS: publish 推到 msgsvr TCP

  Note over MS: ⑤ msgsvr handler + Redis fan-out
  MS->>MS: 解析 PB / 写库<br/>Redis gid→{100001@20001, 100002@20002}

  Note over MS,HB: ⑥ 第一段 fan-out：两帧 AsyncSend 进 BACKEND
  MS->>HB: AsyncSend IM.dest_nid=20001 payload "hi,friend!"
  MS->>HB: AsyncSend IM.dest_nid=20002 payload "hi,friend!"

  Note over HB,HF: ⑦ bridge 下行 × 2
  HB->>HF: bridge AsyncSend(20001)
  HB->>HF: bridge AsyncSend(20002)

  Note over HF,G1: ⑧ FORWARD NID 路由 × 2
  HF->>G1: TCP 下行 → 甲也收到自己的消息
  HF->>G2: TCP 下行 → 乙收到

  Note over G1,A: ⑨ gateway → 浏览器 WS
  G1->>A: WS "hi,friend!"（发者回显）
  G2->>B: WS "hi,friend!"（接收者）
```
...
| 步骤 | 谁 | 做什么 | 用到的关系 |
|------|-----|--------|------------|
| 1 | 甲浏览器 | WS 发群聊 | gateway uid↔ws |
| 2 | gateway-1 | hubclient 上行 | NID 表 20001 |
| 3 | FORWARD | 拼帧 → bridge | — |
| 4 | bridge | Publish → BACKEND | — |
| 5 | BACKEND | 查 SUB 表推 msgsvr | **SUB(0x030B)→msgsvr** |
| 6 | msgsvr handler | 业务处理 + Redis | **gid→成员 NID** |
| 7 | msgsvr | AsyncSend(20001) + AsyncSend(20002) | NID 表 |
| 8 | BACKEND bridge | ReadDestNid → FORWARD | — |
| 9 | FORWARD | 各推 gateway-1 / gateway-2 | **NID 表 20001/20002** |
| 10 | gateway-1 | WS → 甲（回显） | uid↔ws |
| 11 | gateway-2 | WS → 乙（目标） | uid↔ws |
