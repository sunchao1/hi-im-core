...
**编号约定**：下文流程图与 5.1 对照表 **统一为 11 步**（同序号同含义）。
...
sequenceDiagram
  ...
  rect rgb(240,248,255)
    Note over G1,HF: 步骤 2
    G1->>HF: hubclient 上行 cmd=0x030B
  end

  rect rgb(255,248,240)
    Note over HF: 步骤 3
    HF->>HF: Reactor 拼帧 → recvq → Worker
  end

  rect rgb(255,248,240)
    Note over HF,HB: 步骤 4
    HF->>HB: bridge Publish(0x030B)
  end

  rect rgb(240,255,240)
    Note over HB,MS: 步骤 5
    HB->>MS: 查 SUB 表 → publish 推 msgsvr TCP
  end

  rect rgb(248,240,255)
    Note over MS: 步骤 6
    MS->>MS: handler 解析 PB / 写库 / Redis 查成员
  end

  rect rgb(248,240,255)
    Note over MS,HB: 步骤 7
    MS->>HB: AsyncSend(dest=20001) + AsyncSend(dest=20002)
  end

  rect rgb(255,248,240)
    Note over HB,HF: 步骤 8
    HB->>HF: bridge AsyncSend(20001) + AsyncSend(20002)
  end

  rect rgb(255,248,240)
    Note over HF,G2: 步骤 9
    HF->>G1: TCP 下行（NID=20001）
    HF->>G2: TCP 下行（NID=20002）
  end

  rect rgb(240,248,255)
    Note over G1,A: 步骤 10
    G1->>A: WS "hi,friend!"（甲回显）
  end

  rect rgb(240,248,255)
    Note over G2,B: 步骤 11
    G2->>B: WS "hi,friend!"（乙收到）
  end
```
